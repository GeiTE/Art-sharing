    ...
    if(!empty($_POST['search_type'])){
        ...
        switch ($search_type){
            ...
            case "worksshow_data":
                ...
                $array = array();
                $data = "SELECT data_sheet.works_id,data_sheet.works_name,data_sheet.works_introduce,data_sheet.works_datails,data_sheet.works_label,data_sheet.release_time,user_data.user_name,user_data.user_email,user_datails.user_head_portrait FROM data_sheet,user_data,user_datails WHERE data_sheet.works_id = '{$works_id}' AND data_sheet.user_email = user_datails.user_email AND data_sheet.user_email = user_data.user_email";	//查询该用户的具体信息;
                opensql($data);	//连接数据库
                if($result->num_rows >0){	//如果数据大于0，表示查询到数据
